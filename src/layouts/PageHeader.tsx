...
export const PageHeader = () => {
    // state for showing full width
    const [showFullWidthSearch, setShowFullWidthSearch] = useState(false)



    return (<div className="flex gap-10 lg:gap-20 justify-between pt-2 mb-6 mx-4">
        <PageHeaderFirstSecction hidden={showFullWidthSearch} />

        <form className={` gap-4 flex-grow justify-center ${showFullWidthSearch ? "flex" : "hidden md:flex "}`}>
            {showFullWidthSearch && (<Button onClick={() => setShowFullWidthSearch(false)} type="button" size="icon" className="flex-shrink-0">
                <ArrowLeft />
            </Button>)}
...
type PageHeaderFirstSecctionProps = {
    hidden?: boolean
}

export const PageHeaderFirstSecction = ({ hidden = false }: PageHeaderFirstSecctionProps) => {
